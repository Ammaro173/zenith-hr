...
Keep `AGENTS.md` current using conversation logs instead of full rescans.

## Inputs

- Transcript root: `~/.gemini/antigravity/brain/` (Check `.system_generated/logs/` in each conversation folder)
- Existing memory file: `AGENTS.md`
- Incremental index: `.agent/state/continual-learning-index.json`

## Workflow

1. Read existing `AGENTS.md` first.
2. Load incremental index if present.
3. Discover conversation log files and process only:
